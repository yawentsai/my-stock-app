    ...
    with st.expander("🌇 Step 2: 盤後結果統整"):
        ...
        if not waiting.empty:
            with st.form("post_market", clear_on_submit=True):
                ...
                res_pct = st.number_input("結果漲跌 % (支援兩位小數)", step=0.01, format="%.2f")
...
try:
    ...
    for _, row in active_df.iterrows():
        ...
        active_holdings_data.append({'日期': row['日期'], '標的': row['標的'], '代號': row['代號'], '均價': row['成本'], '股數': row['股數'], '投入金額': row['投入金額'], '現價': cp if cp else "讀取中...", '損益金額': p_twd, '損益率%': p_pct})
    ...
    with t1:
        if active_holdings_data:
            # 💡 表格端也強制兩位小數
            st.dataframe(pd.DataFrame(active_holdings_data), use_container_width=True, hide_index=True, column_config={"損益率%": st.column_config.NumberColumn(format="%.2f%%")})
                    ...
                    for _, r in day_df.iterrows():
                        ...
                        <div style="border-left:6px solid {res_c}; padding:15px; background:white; margin-bottom:15px; border-radius:8px; border: 1px solid #eee;">
                            <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                                <span style='background-color:#bdbdbd; color:white; padding:2px 8px; border-radius:4px; font-size:0.85rem;'>{r['操作']}</span>
                ...
                with st.expander(f"📌 {t}"):
                    for _, row in t_df.iterrows():
                        ...
                        <div style="border-left:5px solid {color}; padding:10px; background:#f8f9fa; margin-bottom:10px; border-radius:4px;">
                            <b>{row['日期']} | {row['操作']} | <span style="color:{color};">{row['漲跌%']:.2f}%</span></b><br>
