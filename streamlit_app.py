...
try:
    ...
    # 補齊必要欄位
    req_cols = ['日期', '標的', '代號', '操作', '成本', '股數', '投入金額', '漲跌%', '盤前觀察', '盤後紀錄', '賣出價', '實現損益']
    for c in req_cols:
        if c not in existing_df.columns:
            existing_df[c] = 0.0 if c in ['成本', '股數', '投入金額', '漲跌%', '賣出價', '實現損益'] else ""
except:
...
with st.sidebar:
    ...
    with st.expander("🌅 Step 1: 盤前計畫輸入", expanded=True):
        with st.form("pre_market_form", clear_on_submit=True):
            ...
            p_pre = st.text_area("🔍 盤前核心計畫 (邏輯/條件)")
    ...
    with st.expander("🌇 Step 2: 盤後結果統整"):
        ...
        if not waiting.empty:
            with st.form("post_market_form", clear_on_submit=True):
                ...
                res_post = st.text_area("📝 盤後回饋 (執行檢討)")
    ...
    with st.expander("🛒 實單進場 (庫存)"):
        with st.form("buy_real"):
            ...
            br_name = st.text_input("股票名稱")
            br_symbol = st.text_input("代號")
            br_price = st.number_input("均價", min_value=0.0)
            br_qty = st.number_input("股數", min_value=1, value=100)
    ...
    with st.expander("🗑️ 管理新聞追蹤標的"):
        tracking = existing_df[existing_df['操作'] == '追蹤']['標的'].unique()
        if len(tracking) > 0:
            del_t = st.selectbox("移除標的", tracking)
            if st.button("🚨 執行刪除"):
...
try:
    ...
    for _, row in active_df.iterrows():
        ...
        active_holdings_data.append({'日期': row['日期'], '標的名稱': row['標的'], '代號': row['代號'], '均價': row['成本'], '股數': row['股數'], '投入金額': row['投入金額'], '現價': cp if cp else "讀取中...", '損益金額': p_twd, '損益率%': p_pct})
    ...
    <div style="display:grid; grid-template-columns:repeat(3, 1fr); gap:8px; margin-bottom:20px;">
        <div style="background:#f8f9fa; padding:15px 10px; border-radius:8px; text-align:center; border:1px solid #ddd;"><div style="font-size:1.05rem; color:#555;">總資產權益</div><div style="font-size:1.45rem; font-weight:bold;">${equity:,.0f}</div></div>
        <div style="background:{p_b}; padding:15px 10px; border-radius:8px; text-align:center; border:1px solid #ddd;"><div style="font-size:1.05rem; color:{p_c};">🎯 累計獲利</div><div style="font-size:1.45rem; font-weight:bold; color:{p_c};">${total_profit:,.0f}</div></div>
        <div style="background:#f8f9fa; padding:15px 10px; border-radius:8px; text-align:center; border:1px solid #ddd;"><div style="font-size:1.05rem; color:#555;">📈 ROI</div><div style="font-size:1.45rem; font-weight:bold;">{(total_profit/INITIAL_CAPITAL)*100:.2f}%</div></div>
        <div style="background:#fff3e0; padding:15px 10px; border-radius:8px; text-align:center; border:1px solid #ddd;"><div style="font-size:1.05rem; color:#e65100;">已投入資金</div><div style="font-size:1.45rem; font-weight:bold; color:#e65100;">${used_cap:,.0f}</div></div>
        <div style="background:#f8f9fa; padding:15px 10px; border-radius:8px; text-align:center; border:1px solid #ddd;"><div style="font-size:1.05rem; color:#555;">可用現金</div><div style="font-size:1.45rem; font-weight:bold;">${rem_cap:,.0f}</div></div>
        <div style="background:#f8f9fa; padding:15px 10px; border-radius:8px; text-align:center; border:1px solid #ddd;"><div style="font-size:1.05rem; color:#555;">利用率</div><div style="font-size:1.45rem; font-weight:bold;">{(used_cap/(INITIAL_CAPITAL+total_realized))*100:.1f}%</div></div>
    </div>
    """, unsafe_allow_html=True)

    # 2. 停利提示
    if ready_to_sell:
        st.error(f"🚨 停利提示：{', '.join(ready_to_sell)}")
    else:
        st.success("✅ 持股獲利尚未達標，請耐心持有。")
    ...
    with t2:
        news_list = sorted(list(set(list(active_df['標的'].unique()) + list(df[df['操作'] == '追蹤']['標的'].unique()))))
        for s in news_list:
        ...
        if not completed.empty:
            for d in sorted(completed['日期'].unique(), reverse=True):
                ...
                for _, r in completed[completed['日期']==d].iterrows():
                    ...
                    st.markdown(f"""
                    <div style="border-left:6px solid {res_c}; padding:15px; background:white; margin-bottom:20px; border-radius:8px; border: 1px solid #eee;">
                        ...
                        <div style="background:#f8f9fa; padding:10px; border-radius:6px; margin-bottom:8px;">
                            <div style="color:#666; font-size:0.85rem;">🔍 盤前計畫</div><div>{r['盤前觀察']}</div>
                        </div>
                        <div style="background:#fff; padding:10px; border-radius:6px; border:1px dashed #ddd;">
                            <div style="color:#666; font-size:0.85rem;">📝 盤後回饋</div><div>{r['盤後紀錄']}</div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
        ...
        if not completed.empty:
            for t in sorted(completed['標的'].unique()):
                with st.expander(f"📌 {t}"):
                    st.dataframe(completed[completed['標的'] == t])

    # 4. 底部勝率
    st.divider()
    c_buy = completed[completed['操作'] == '買進']
    w_r = (len(c_buy[c_buy['漲跌%'] > 0]) / len(c_buy) * 100) if not c_buy.empty else 0.0
    st.markdown(f"<span style='font-size:2.5rem; font-weight:bold; color:#2196f3;'>{w_r:.1f}%</span> 實際勝率", unsafe_allow_html=True)
    if not df.empty:
        fig = px.pie(df, names='操作', hole=0.4, color_discrete_sequence=['#2196f3', '#bdbdbd', '#ffeb3b'])
        st.plotly_chart(fig, use_container_width=True)

except Exception as e: st.info(f"系統啟動中... ({e})")
